...
fn patch_space_service_controls() {
    ...
    if !text.contains("\"set_space_addon_enabled\" => {") {
        text = text.replace(
            ...
            "            \"leave_space\" => {\n                self.log(\"Left space.\");\n                self.send_job(ApiJob::Spaces);\n                self.send_job(ApiJob::Addons);\n            }\n            \"set_space_addon_enabled\" => {\n                self.log(\"Local space add-on setting updated.\");\n                self.send_job(ApiJob::Spaces);\n                self.send_job(ApiJob::Addons);\n            }",
        );
    }
