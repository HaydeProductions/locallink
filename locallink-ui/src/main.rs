...
impl LocalLinkUi {
    fn new() -> Self {
        ...
        let mut app = Self {
            ...
            add_name: String::new(),
            add_mac: String::new(),
            event_filter_service: String::new(),
        };
    ...
    fn refresh_visible(&mut self) {
        ...
        match self.screen {
            ...
            Screen::Addons => self.send_job(ApiJob::Addons),
            Screen::Activity => {
                let service = if self.event_filter_service.trim().is_empty() {
                    None
                } else {
                    Some(self.event_filter_service.trim().to_string())
                };
    ...
    fn send_job(&mut self, job: ApiJob) {
        ...
        if let Err(e) = self.tx.send(job) {
            ...
            self.log(format!("UI worker unavailable: {e}"));
        }
    }
    ...
    fn apply_paths(&mut self, v: Value) {
        if let Some(data) = v.get("data") {
            self.paths = Some(Paths {
                ...
                trusted_peers_file: str_field(data, "trusted_peers_file"),
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        if self.core_online() {
                            pill(ui, "Online", color_success());
                        } else {
                            pill(ui, "Offline", color_error());
                        }

                        if !self.connections.is_empty() {
                            pill(
                                ui,
                                &format!("{} connected", self.connections.len()),
                                color_accent(),
                            );
                        }

                        if self.loading_count > 0 {
                            pill(ui, "Updating", color_warning());
                        }

                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if ui.add(icon_button("⚙ Settings")).clicked() {
        ...
        egui::TopBottomPanel::top("tabs").show(ctx, |ui| {
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        nav_tab(ui, &mut self.screen, Screen::Discover, "Discover");
                        nav_tab(ui, &mut self.screen, Screen::Devices, "Devices");
                        nav_tab(ui, &mut self.screen, Screen::Addons, "Add-ons");
                        nav_tab(ui, &mut self.screen, Screen::Activity, "Activity");
                    });
                });
        });
...
impl LocalLinkUi {
    ...
    fn screen_discover(&mut self, ui: &mut egui::Ui) {
        page_title(
            ui,
            "Discover",
            "Nearby devices are shown here. Add a device by MAC address before connecting.",
        );
        ...
        if !self.core_online() {
            notice(
                ui,
                "LocalLink Core is not running",
                "Start the core to discover nearby devices.",
                color_error(),
            );
            return;
        }
        ...
        if self.peers.is_empty() {
            notice(
                ...
                "Start LocalLink on another device connected to the same local network.",
                color_warning(),
            );
            return;
        }
        ...
        for peer in self.peers.clone() {
            card(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ui.heading(&peer.device_name);
                        ui.label(
                            egui::RichText::new(format!(
                                "Seen {} ago",
                                format_duration_ms(peer.last_seen_ms_ago)
                            ))
                            .color(color_muted()),
                        );
                    });
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if peer.connected {
                            pill(ui, "Connected", color_success());
                        } else if peer.trusted {
                            pill(ui, "Trusted", color_accent());
                        } else {
                            pill(ui, "Unknown", color_warning());
                        }
                    });
                });
                ...
                if !primary_mac.is_empty() {
                    ui.label(format!("MAC: {primary_mac}"));
                }

                ui.horizontal(|ui| {
                    if !peer.trusted {
                        if ui.add(primary_button("Add trusted device")).clicked() {
                            self.add_name = peer.device_name.clone();
                            self.add_mac = primary_mac.clone();
                ...
                if self.show_advanced {
                    ui.separator();
                    readonly_line(ui, "Device ID", &peer.device_id);
                    ui.label(format!("Address: {}", peer.addr));
                    ui.label(format!("All MACs: {}", peer.macs.join(", ")));
                }
            });
        ...
        card(ui, |ui| {
            ui.heading("Add manually");
            ui.label("Enter a MAC address if the device is not currently visible.");
    ...
    fn screen_devices(&mut self, ui: &mut egui::Ui) {
        page_title(
            ui,
            "Devices",
            "Trusted devices appear here. Click Connect when you want to open a secure connection.",
        );
        ...
        if self.trusted.is_empty() {
            notice(
                ...
                "Use Discover to add a nearby device by MAC address.",
                color_warning(),
            );
            return;
        }
        ...
        for trusted in self.trusted.clone() {
            ...
            card(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ui.heading(&trusted.name);
                        ui.label(
                            egui::RichText::new(format!("MAC: {}", trusted.macs.join(", ")))
                                .color(color_muted()),
                        );
                    });
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if connected.is_some() {
                            ...
                            pill(ui, "Available", color_accent());
                        } else {
                            pill(ui, "Offline", color_muted());
                        }
                    });
                });
                ...
                if self.show_advanced {
                    ...
                    if let Some(device_id) = &trusted.device_id {
                        readonly_line(ui, "Registered device ID", device_id);
                    }
                    if let Some(peer) = nearby_peer {
                        readonly_line(ui, "Nearby device ID", &peer.device_id);
                        ui.label(format!("Address: {}", peer.addr));
                    }
                }
            });
    ...
    fn screen_addons(&mut self, ui: &mut egui::Ui) {
        page_title(
            ui,
            "Add-ons",
            "Turn LocalLink features on or off. Add-ons use connected devices in the background.",
        );
        ...
        for addon in &mut self.addons {
            card(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if self.addon_processes.contains_key(&addon.id) {
                            pill(ui, "Running", color_success());
                        } else if addon.enabled {
                            pill(ui, "Enabled", color_warning());
                        } else {
                            pill(ui, "Off", color_muted());
                        }
                    });
                });
                ...
                if self.show_advanced {
                    ui.separator();
                    ui.label(format!("ID: {}", addon.id));
                    ui.label(format!("Executable: {}", addon.executable));
                    ui.label(format!("Services: {}", addon.services.join(", ")));
                    ui.label(format!("Folder: {}", addon.addon_dir));
                    ui.label(format!("Manifest: {}", addon.manifest_path));
                }
            });
    ...
    fn screen_activity(&mut self, ui: &mut egui::Ui) {
        page_title(
            ui,
            "Activity",
            "Recent add-on and device activity appears here.",
        );
        ...
        card(ui, |ui| {
            ui.horizontal(|ui| {
                ui.label("Filter");
                ui.text_edit_singleline(&mut self.event_filter_service);

                if ui.add(primary_button("Check activity")).clicked() {
                    let service = if self.event_filter_service.trim().is_empty() {
                        None
                    } else {
                        Some(self.event_filter_service.trim().to_string())
        ...
        if self.events.is_empty() {
            notice(
                ui,
                "No activity yet",
                "Feature messages and replies will appear here.",
                color_warning(),
            );
            return;
        }
            ...
            for event in self.events.iter().rev() {
                card(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        pill(ui, &event.service, color_accent());
                    });
                    ...
                    if self.show_advanced {
                        ui.separator();
                        ui.label(format!("Kind: {}", event.kind));
                        readonly_line(ui, "Peer ID", &event.peer_id);
                        if let Some(channel_id) = &event.channel_id {
                            readonly_line(ui, "Channel ID", channel_id);
                        }
                        if let Some(message_id) = &event.message_id {
                            readonly_line(ui, "Message ID", message_id);
                        }
    ...
    fn settings_window(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Settings")
            ...
            .resizable(true)
            .show(ctx, |ui| {
                ui.heading("LocalLink Settings");
                ui.label(
                    egui::RichText::new(
                        "Simple controls first. Technical details are hidden unless enabled.",
                    )
                    .color(color_muted()),
                );
                ...
                card(ui, |ui| {
                    ...
                    if self.show_advanced {
                        ...
                        if let Some(status) = &self.status {
                            readonly_line(ui, "API", &status.api_addr);
                            readonly_line(ui, "Device ID", &status.device_id);
                        }

                        if let Some(paths) = &self.paths {
                            ui.separator();
                            readonly_line(ui, "AppData", &paths.app_dir);
                            readonly_line(ui, "Config", &paths.config_file);
                            readonly_line(ui, "Trusted devices", &paths.trusted_devices_file);
                            readonly_line(ui, "Trusted peers", &paths.trusted_peers_file);
                            readonly_line(ui, "Add-ons", &paths.addons_dir);
                            readonly_line(ui, "Logs", &paths.logs_dir);
                            readonly_line(ui, "Runtime", &paths.runtime_dir);
                            readonly_line(ui, "State", &paths.state_dir);
                            readonly_line(ui, "Lock", &paths.lock_file);
                        }
                    } else {
                        ui.label(
                            egui::RichText::new(
                                "Technical IDs, paths, services, and raw details are hidden.",
                            )
                            .color(color_muted()),
                        );
                    }
...
fn nav_tab(ui: &mut egui::Ui, current: &mut Screen, tab: Screen, label: &str) {
...
fn primary_button(text: &str) -> egui::Button<'_> {
    egui::Button::new(egui::RichText::new(text).color(color_text()).strong())
...
fn page_title(ui: &mut egui::Ui, title: &str, subtitle: &str) {
    ui.heading(egui::RichText::new(title).color(color_text()).size(30.0));
    ui.label(egui::RichText::new(subtitle).color(color_muted()));
}
...
fn notice(ui: &mut egui::Ui, title: &str, body: &str, color: egui::Color32) {
    card(ui, |ui| {
        ...
        ui.label(egui::RichText::new(body).color(color_muted()));
    });
}
...
fn pill(ui: &mut egui::Ui, text: &str, color: egui::Color32) {
    egui::Frame::none()
        ...
        .stroke(egui::Stroke::new(1.0, color.linear_multiply(0.8)))
...
fn toggle_switch(ui: &mut egui::Ui, value: &mut bool) -> egui::Response {
    let desired_size = egui::vec2(56.0, 30.0);
    ...
    let x = egui::lerp((rect.left() + 15.0)..=(rect.right() - 15.0), t);
    let center = egui::pos2(x, rect.center().y);

    ui.painter().circle_filled(center, 10.5, knob);
